...
To connect to a MariaDB database, provide the following details:
...
You can connect to remote MariaDB instances through an SSH tunnel. Configure the SSH host, port, user, and authentication method (password or private key) in the connection dialog.

### SSL / TLS

Zequel supports SSL/TLS connections to MariaDB. You can provide a CA certificate, client certificate, and client key to establish a secure connection.
...
MariaDB events (scheduled tasks) are fully supported. You can browse event definitions, schedules, and statuses. This is a feature shared with MySQL but not available in other supported databases.
