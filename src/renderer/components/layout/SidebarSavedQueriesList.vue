...
<template>
  <div class="space-y-0.5 py-2">
    ...
    <div class="flex items-center justify-end px-2 pb-1">
      <Button variant="ghost" size="sm" class="h-6 px-2 text-xs text-muted-foreground" @click="emit('new')">
    ...
    <template v-else>
        ...
        <ContextMenu>
          <ContextMenuTrigger as-child>
            ...
            >
              ...
              <!-- Description -->
              <p v-if="query.description" class="text-xs text-muted-foreground mt-0.5 truncate pl-5">{{ query.description }}</p>
              <!-- SQL preview -->
              <p class="text-xs font-mono text-foreground/50 mt-0.5 line-clamp-1 break-all pl-5">{{ truncate(query.sql, 100) }}</p>
            </div>
          </ContextMenuTrigger>
          <ContextMenuContent>
      ...
      <!-- Empty state -->
      <div v-if="!loading && queries.length === 0" class="flex flex-col items-center justify-center py-8 text-center text-muted-foreground">
